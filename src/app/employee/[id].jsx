...
const EmployeeDetails = () => {
  ...
  const fetchEmployeeDetails = async () => {
    try {
      setRefreshing(true);
      const response = await fetch(`${API_URL}/api/employees/report/payments`, {
        headers: {
      ...
      if (response.ok) {
        ...
        showMessage('error', 'Failed to fetch employee details');
      }
    } catch (error) {
      showMessage('error', 'Error fetching employee details');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
                ...
                {activeTab === 'leaves' && (
                  <div>
                    ...
                    {employee.employee?.leaves?.length > 0 ? (
                      <div className="space-y-3">
                        {employee.employee.leaves.map((leave, index) => (
                          <div key={index} className="p-4 bg-gray-50 rounded-lg">
                            ...
                            <div className="text-sm text-gray-600">
                              Duration: {Math.ceil((new Date(leave.endDate) - new Date(leave.startDate)) / (1000 * 60 * 60 * 24)) + 1} days
                            </div>
                          </div>
                        ))}
                      </div>
